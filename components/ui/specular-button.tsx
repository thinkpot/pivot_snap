...
type ButtonVariant = 'bull' | 'bear' | 'outline'
...
// PivotSnap palette: bull green #22c55e, bear red #ef4444, ink #080b12
const VARIANTS: Record<ButtonVariant, { tint: string; tintOpacity: number; textColor: string; lineColor: string; baseColor: string }> = {
  bull: { tint: '#22c55e', tintOpacity: 0.12, textColor: '#dcfce7', lineColor: '#4ade80', baseColor: '#166534' },
  bear: { tint: '#ef4444', tintOpacity: 0.1, textColor: '#fee2e2', lineColor: '#f87171', baseColor: '#7f1d1d' },
  outline: { tint: '#ffffff', tintOpacity: 0.04, textColor: '#e2e8f0', lineColor: '#ffffff', baseColor: '#475569' },
}
...
export function SpecularButton({
  ...
  const resolved = {
    ...
    tintOpacity: tintOpacity ?? theme.tintOpacity,
    textColor: textColor ?? theme.textColor,
    lineColor: lineColor ?? theme.lineColor,
  ...
  useEffect(() => {
    ...
    // The page can host many of these canvases; once the browser's WebGL
    // context budget (~16, shared with every other canvas on the page) is
    // exhausted, ogl's Renderer throws while attaching itself to a null gl
    // context. Degrade to the plain CSS button instead of crashing the page.
    ...
    // Only burn a rAF loop while the button is actually on screen — the home
    // page renders several of these and each one holds its own WebGL context.
